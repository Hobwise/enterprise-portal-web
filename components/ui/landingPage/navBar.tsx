    ...
    <div className={cn(openNav && 'w-full bg-white h-screen relative')}>
      {openNav && (
        <Image
          ...
          alt=''
          className='absolute top-0 right-0 w-[90%] lg:hidden'
          priority
        />
      )}
      ...
      >
        <Link href={HOME_URL}>
          <Image
            src={type === 'default' ? HobinkLogo : HobinkLogoWhite}
            alt='Hobwise logo'
            width={type === 'default' ? 120 : 240}
          />
        </Link>
        <nav
          className={cn(
      ...
      {openNav && (
        <div className='my-6 px-6 space-y-6 lg:hidden'>
          ...
          <nav
            className={cn(
              'block text-left text-[32px] space-y-4 text-[#616B7C] font-bricolage_grotesque'
            )}
          >
            {navItem.map((each) => {
              const isActive =
