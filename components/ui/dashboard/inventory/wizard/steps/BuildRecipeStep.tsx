...
const BuildRecipeStep: React.FC<BuildRecipeStepProps> = ({
  ...
  const handleSaveRecipe = async () => {
    ...
    try {
      ...
      const defaultUnitId = Array.isArray(units) && units.length > 0 ? units[0].id : '';

      const payload: CreateRecipePayload = {
        name: `${createdItemName} Recipe`,
        producedInventoryItemID: createdItemId,
        outputQuantity: 1,
        outputQuantityUnitId: defaultUnitId,
        recipeType: 0,
        isActive: true,
      ...
      const response = await createRecipe(businessId, payload);
      ...
      if (response?.data?.isSuccessful) {
        ...
        notify({ title: 'Error!', text: response?.data?.error || 'Failed to create recipe', type: 'error' });
      }
    } catch (error) {
    ...
    <div className="min-h-[70vh] bg-[#EBE8F9] rounded-2xl p-6 sm:p-10 flex flex-col">
      ...
      <div className="flex justify-between items-center w-full mt-auto">
        ...
        <button
          ...
          disabled={loading || recipeDetails.length === 0}
