...
const OrdersList: React.FC<OrdersListProps> = ({
  ...
  const [isOpenUpdateOrder, setIsOpenUpdateOrder] =
    React.useState<boolean>(false);
  const [isOpenCheckoutModal, setIsOpenCheckoutModal] =
    React.useState<boolean>(false);
  const [checkoutSelectedItems, setCheckoutSelectedItems] = React.useState<
    any[]
  >([]);
  ...
  const orderDetails = React.useMemo(() => {
    ...
    if (statusFilter !== null) {
      filteredOrders = orders.filter(
        (order: OrderItem) => order.status === statusFilter
      );
    }
  ...
  const togglePaymentModal = (order: OrderItem) => {
    ...
    setAmountPaid(
      (order.amountRemaining || order.totalAmount).toLocaleString("en-US")
    );
  };

  const handlePaymentClick = async (methodId: number) => {
    if (methodId === 3) {
      // Pay Later - handle properly depending on current page
      try {
        if (pathname === "/dashboard/orders") {
          ...
          // Invalidate and refetch in the background to avoid UI delay
          ...
          // Not on orders page - redirect like checkoutModal does
  ...
  const finalizePayment = async () => {
    ...
    const totalDue = singleOrder.amountRemaining ?? singleOrder.totalAmount;
    let finalAmountPaid = totalDue;
    const sysRef = (): number => Math.floor(1e9 + Math.random() * 9e9);
  ...
  const renderCell = React.useCallback(
    (order: OrderItem, columnKey: string) => {
...

                    {
                      ((role === 0 ||
                        ...
                        options.includes("Confirm Payment") && (
                          <DropdownItem
                            key="confirm-payment"
                            onClick={() => togglePaymentModal(order)}
            ...
            >
              {(order: OrderItem) => (
                ...
                >
                  {(columnKey) => (
                    <TableCell>
                      {renderCell(order, String(columnKey))}
                    </TableCell>
                  )}
                </TableRow>
              )}
