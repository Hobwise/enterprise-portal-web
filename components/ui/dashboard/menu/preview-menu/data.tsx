...
export const togglePreview = (activeTile: string): PreviewStyles => {
  const previewStyles: {
    ...
    "List left": {
      ...
      // classic list with small circular image on the left - mobile view only
    ...
    "List Right": {
      ...
      // compact horizontal list (image on the right) - mobile view only
      main: "relative px-4 grid grid-cols-1 gap-3",
      imageContainer:
        "w-[60px] h-[60px] flex-shrink-0 rounded-lg overflow-hidden",
      textContainer: "flex-1 min-w-0",
    ...
    "Single column 1": {
      ...
      textContainer: "p-4",
      // full-width large card - mobile view only
    ...
    "Single column 2": {
      // 3-column grid - mobile view (matches create order mobile)
      ...
      imageContainer: "w-full",
      main: "grid grid-cols-3 gap-3 px-4",
      divider: false,
      text3: "Chinese fried rice served with fresh vegetable salad",
      chipPosition: "top-2 left-2",
    },
    "Double column": {
      // 2-column grid - mobile view (matches create order mobile)
      container: "flex-col bg-white rounded-xl overflow-hidden shadow-sm",
