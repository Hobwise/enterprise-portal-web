...
export default function BookReservationPage({
  ...
  const {
    ...
    queryFn: () =>
      fetchAvailability({
        ...
        RequestDate: details?.bookingDateTime,
      }),
  ...
  const handleBookReservation = async (
    ...
    if (!details.bookingDateTime || !Array.from(selectedTime).join(", ")) {
      setError((prev) => ({
        ...prev,
        bookingDateTime: "Preferred date and time is compulsory",
      }));
    }
  ...
  function checkAvailability() {
    const result = generateTimeSlots(
      reservation?.startTime || "10:00:00",
      reservation?.endTime || "23:59:00",
      1
    ).map((time: string) => {
      if (
        formattedTimeSlots?.find((each: { timeSlot: string }) =>
          each ? convertToMinutes(each?.timeSlot) >= convertToMinutes(time) : ""
        )
      ) {
        return formattedTimeSlots?.find((each: { timeSlot: string }) =>
          each ? convertToMinutes(each?.timeSlot) >= convertToMinutes(time) : ""
        );
      } else {
  ...
  useEffect(() => {
    if (selectedTime) {
      setDetails((prev) => ({
        ...prev,
        time: Array.from(selectedTime).join(", "),
      }));
        ...
        <section className="space-y-10">
          <div className="space-y-2 text-[#161618]">
            <div className="flex items-center space-x-2">
              <h1 className="text-[32px] font-bricolage_grotesque">
                Book Reservation
              </h1>
            </div>
            <p className="text-sm">
              Please provide your details below to book this reservation
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20">
            <div className="space-y-8">
                    ...
                    {isLoadingTimes ? (
                      <div className="mt-4 flex items-center text-dark">
                        ...
                        <p className="text-sm font-light italic">
                          Please wait..., loading available time
                        </p>
                      </div>
                    ) : (
                      <React.Fragment>
                          ...
                          <div>
                            {checkAvailability().map((each: any) => (
                              <Tooltip
                                content={
                                  <p className="text-[#000]">
                                    {each.quantity > 0
                                      ? `${each.quantity} Quantity Available`
                                      : "Not available"}
                                  </p>
                                }
                                color="default"
                              >
                                <div
                                  style={
                                    currentSelection === each.timeSlot
                                      ? bgColorStyle
                                      : {
                                          ...textColorStyle,
                                          ...borderColorStyle,
                            ...
                            <div className="space-y-4 mt-6">
                              <p className="text-[#161618] text-xs font-medium">
                                ...
                                <span className="font-bold">
                                  {reservation?.startTime || "10:00AM"} to{" "}
                                  {reservation?.endTime || "11:59PM"}
                                </span>
                              </p>
              ...
              <div className="space-y-10">
                <div className="bg-[#F0F2F4] p-4 text-[#5A5A63] space-y-2 rounded-lg">
                  <p className="font-semibold text-lg" style={textColorStyle}>
                    Important Note
                  </p>
                  <p className="text-sm">
            ...
            <div className="lg:space-y-6 space-y-6 lg:-mt-16">
              <div className="space-y-1.5">
                <p className="text-lg font-medium text-[#161618]">
                ...
                <div className="border border-[#E4E7EC] py-4 px-5 rounded-lg space-y-4">
                  <div className="lg:flex border-b border-b-[#E4E7EC] pb-6 items-start lg:space-x-4 space-y-2.5 lg:space-y-0">
                    <div className="w-full lg:w-[25%] h-fit rounded-md">
                      {reservation?.image ? (
                        ...
                        <div
                          className="w-full h-[75px] lg:h-[150px] rounded-md bg-[#DDDCFE] font-medium text-[40px] lg:text-[80px] font-bricolage_grotesque flex items-center justify-center"
                  ...
                  <div className="space-y-4 border-b border-b-[#E4E7EC]  pb-6">
                    <div className="text-sm flex justify-between">
                      ...
                      <p className="text-[#808B9F]">
                        ₦{formatNumber(reservation?.minimumSpend || 0)}.00
                      </p>
                    </div>
                    ...
                    <div className="text-sm flex justify-between">
                      <div className="text-[#404245] flex space-x-2 items-center">
                        ...
                        <Tooltip
                          content={
                            <p className="text-[#000]">Reservation cost</p>
                          }
                        >
                          <InfoCircle />
                        </Tooltip>
                      </div>
                      <p className="text-[#404245]">
                        ₦{formatNumber(reservation?.reservationFee || 0)}.00
                      </p>
                    </div>
                    ...
                    <div className="text-sm flex justify-between">
                      <div className="text-[#404245] flex space-x-2 items-center">
                        ...
                        <Tooltip
                          content={
                            <p className="text-[#000]">Total quantity</p>
                          }
                        >
                          <InfoCircle />
                        </Tooltip>
                      ...
                      <div className="flex space-x-4 text-[#000] items-center">
                        ...
                        <button
                          ...
                          role="button"
                          onClick={() => {
                            quantity >= selectedPeriod?.quantity
                              ? null
                              : setQuantity((prev) => prev + 1);
                          }}
                        >
                    ...
                    {reservation.numberOfSeat > 0 && (
                      <p className="text-xs italic text-dark">
                        Note: Max. Number of Seats is {reservation.numberOfSeat}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="font-bold flex justify-between text-[#404245]">
                    ...
                    <p className="opacity-20">
                      ₦
                      {formatNumber(
                        Number(reservation?.reservationFee || 0) * quantity
                      )}
                      .00
                    </p>
                  </div>
                  ...
                  <div className="bg-[#F0F2F4] p-4 text-[#5A5A63] flex items-baseline space-x-2 rounded-lg">
                    ...
                    <p>
                      The minimum spend is the amount you’re required to spend
              ...
              <div className="text-[#161618] border border-[#E4E7EC] py-2 px-4 rounded-lg space-y-2">
                <p className="font-medium">
                  Locate{" "}
                  <span className="uppercase font-medium text-[18px]">
        ...
        <ModalContent>
          <ModalBody className="text-center px-10 py-8">
            <Image
              ...
              className="mx-auto"
            />
            <h1 className="text-[#000] text-xl font-bricolage_grotesque">
              Reservation has been booked!
            </h1>
            <p className="text-[#475367] font-satoshi">
              You’ll get an email after details of your reservation has been
              confirmed.
            </p>
            <CustomButton
              className={btnClassName}
